...
- 🤖 **LLM Wiki** — 接入 DeepSeek API，以 vault 笔记、待办和日程为基础构建个人知识库；Agent 模式支持 5 个工具（时间/待办查询/待办添加/统计分析/网络搜索）；Memory-Augmented 架构（4 层记忆 + 语义检索 + 自适应路由）；3 闭环自演化系统；每日 13:00 自动维护
...
### LLM Wiki（Memory-Augmented Agent）

- 🤖 **知识库构建** — 扫描 vault 中所有 Markdown 笔记，逐篇调用 DeepSeek API 生成摘要，自动构建结构化 wiki（目录索引、知识总览、概念交叉引用）。维护完成后重建 TF-IDF 向量索引用于语义检索
- 💬 **Agent 对话** — Memory-Augmented 架构：工具路由 → 语义检索 → 4 层记忆 → LLM 推理 → 工具执行 → 记忆写入完整管线。流式逐字输出，markdown 实时渲染
- 🧠 **记忆系统** — 4 层结构化记忆：短期对话记忆、情景记忆（事件/目标/决策，含衰减与强化）、用户画像（结构化属性+置信度）、工具记忆（使用统计+有效性追踪）
- 🔄 **自演化系统** — 3 个闭环：记忆演化（自动衰减/强化/合并去重）、路由学习（自适应工具选择阈值与策略权重）、RAG 优化（检索反馈权重调整与查询聚类）
- 🖱 **链接操作** — 点击回复中的链接弹出菜单，可选择"复制链接"或"在浏览器中打开"
- 🌐 **网络搜索** — `web_search` 工具通过 DuckDuckGo 搜索互联网实时信息，知识库无法回答时自动联网查询，结果附带来源链接
