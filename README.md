...
- 🤖 **LLM Wiki** — 5 层自我改进认知系统：记忆(Markdown) → 概念(自动提取+链接) → 推理(概念图+3策略) → 反馈(权重学习) → 策略(全局认知控制)；Agent 模式支持 5 个工具；全部 Markdown 持久化，用户可见可编辑；每日 13:00 自动维护
...
- 🌐 **网络搜索** — `web_search` 工具通过 DuckDuckGo 搜索互联网实时信息（JSON API + HTML 双层 fallback），结果附带来源链接
