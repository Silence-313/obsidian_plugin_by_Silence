...
- 🖱 **自由拖拽** — Header 栏拖拽移动浮窗任意位置，右下角手柄缩放尺寸，位置和大小在会话中保持
- 🔘 **最小化 FAB** — 点击 `—` 最小化为右下角可拖拽浮动按钮（💬），点击恢复；切换笔记/视图时自动最小化/恢复
- 📄 **笔记内容同步** — 开关控制是否将当前笔记完整内容注入每次提问上下文（截断 4000 字符），Agent 可引用笔记细节回答
- ✏️ **直接编辑笔记** — Agent 拥有 4 个编辑器工具：在光标处插入文本、查找替换、追加到末尾、获取选中文本，可直接修改当前笔记
